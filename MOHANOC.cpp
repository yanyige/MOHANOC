        ...
        for(int ii=0; ii<meshfactor; ii++)
        {
            ...
            if(isdepcopy[flag_end[ii]][position]==0)
            {
                ...
                {   //需要通信
                    ...
                    {   flag_com[ii]=1;
                        Eb+=c[kk][position]*(D[taks_in_machine[kk]][taks_in_machine[position]]*Rb_dis+(D[taks_in_machine[kk]][taks_in_machine[position]]+1)*Rb_node;
                        ltemp=taoend[flag_end[ii]][kk]+c[kk][position]*(D[taks_in_machine[kk]][taks_in_machine[position]]*t_dis+(D[taks_in_machine[kk]][taks_in_machine[position]]+1)*t_node)+2;
                        if(ltemp>Lstart)
                        {
                            Lstart=ltemp;
                        }
...
void greedy_with_topo() {
    ...
    memset(onMachine, 0, sizeof(onMachine));
