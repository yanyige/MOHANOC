...
int machineMap[5][16] = {{0}, {0}, {0}, {4, 1, 2, 5, 8, 7, 6, 3, 0}, {9, 5, 6, 10, 14, 13, 12, 8, 4, 0, 1, 2, 3, 7, 11, 15}};
...
bool taskUsed[MAXN];
int mesh[meshfactor*meshfactor]={0};//4*4 2D mesh
...
//    if((a->makespan >= b->makespan && a->workload >= b->workload) || (a->workload > b->workload && a->makespan >= b->makespan)) return 2;//b dominates a
    if(a->makespan <= b->makespan && a->p_contention <= b->p_contention && a->energy<=b->energy) return 1;//a dominates b
	if(a->makespan >= b->makespan && a->p_contention >= b->p_contention && a->energy>=b->energy) return 2;//a dominates b
...
//                        }
						...
						if(temp_individual.makespan <= makespan && temp_individual.p_contention <= p_contention && temp_individual.energy <= energy){
							if(temp_individual.makespan==makespan && temp_individual.p_contention==p_contention && temp_individual.energy==energy){
...
//        printf("best_individual.makespan = %.2lf individual->makespan = %.2lf\n", best_individual.makespan, individual->makespan);
       // if((best_individual.makespan <= individual->makespan && best_individual.workload < individual->workload) || (best_individual.makespan < individual->makespan && best_individual.workload <= individual->workload)) {
          if(best_individual.makespan <= individual->makespan && best_individual.p_contention <= individual->p_contention && best_individual.energy<=individual->energy){
		  	if(best_individual.makespan == individual->makespan && best_individual.p_contention== individual->p_contention && best_individual.energy==individual->energy)
			{
				}
			else{
...
bool group_cmp(const group &a,const group &b) {
...
void normalizeAssign(int onMachine[]) {
    groups.clear();
    int allMachine = -inf;
    vector<int> temp;
    for(int i = 0; i < n ; i ++) {
        if(allMachine < onMachine[i]) {
            allMachine = onMachine[i];
        }
    }

    for(int i = 0; i < allMachine + 1; i ++) {
        _group.id = i;
        _group.number = 0;
        for(int j = 0; j < n; j ++) {
            if(onMachine[j] == i) {
    ...
    while(allMachine >= m - 1) {
        sort(groups.begin(), groups.end(), group_cmp);
        for(int i = 0; i < allMachine + 1; i ++) {
            printf("%d %d\n", groups[i].id, groups[i].number);
        }
         //merge
        printf("begin merge\n");
        for(int i = 0; i < n; i ++) {
            if(onMachine[i] == groups[0].id) {
                onMachine[i] = groups[1].id;
            }
        }
        groups[1].number += groups[0].number;
        groups.erase(groups.begin());
        allMachine --;
//        for(int i = 0; i < n; i ++) {
//            printf("%d ", onMachine[i]);
//        }printf("\n");
    }
    for(int i = 0; i < n; i ++) {
        printf("%d ", onMachine[i]);
    ...
    for(int i = 0; i < machines.size(); i ++) {
        for(int j = 0; j < machines[i].size(); j ++) {
            Collection[0].machine[machineMap[m][i]][j] = machines[i][j];
        }
    }
}
...
printf("???\n");
    for(int i=0;i<meshfactor*meshfactor;i++){
        for(int j=0;j<meshfactor*meshfactor;j++){
            D[i][j]=abs(i/meshfactor-j/meshfactor)+abs(i%meshfactor-j%meshfactor);
            //printf("%d ",D[i][j]);
        }
    }
//读文件
