...
const int machineMap[5][16] = {{0}, {0}, {0}, {4, 1, 2, 5, 8, 7, 6, 3, 0}, {9, 5, 6, 10, 14, 13, 12, 8, 4, 0, 1, 2, 3, 7, 11, 15}};
...
const double Rb_dis=0.2;//线路能耗
const double Rb_node=0.4;//节点能耗
const double cominitcost=2;
const double t0=200.0;//时间阈值
const double t_penalty=25;//时间惩罚
const double extra_E=1;//能耗惩罚
const int meshfactor=3;//3*3
const double t_dis=0.5;//线路传输速度
const double t_node=1.0;//节点传输速度
const double xi= 100.0;//冲突间隔
const double xita=0.9;//xita-dominance
int mycycle;
double pp[MAXN][MAXN];
double com_sum=0;
                                ...
                                else
                                {
                                    if(x_temp==0)
                                    {
                                        ...
                                        if(task_in_machine[kk]/meshfactor-task_in_machine[position]/meshfactor>0)
                                        {
                                            ...
                                            {
                                                ltemp+=path[source_path][source_path-meshfactor][1]+c[kk][position]*(t_node+t_dis);
                                            ...
                                            else
                                            {
                                                ...
                                                ltemp+=c[kk][position]*(t_node+t_dis);
                                                path[source_path][source_path-meshfactor][1]=ltemp;
                                    ...
                                    else  //包括y_temp=0和 x,y都不为0，优先走x轴
                                    {
                                        ...
                                        if(task_in_machine[kk]%meshfactor-task_in_machine[position]%meshfactor>0)
                                        {
                                            ...
                                            {
                                                ltemp+=path[source_path][source_path-1][1]+c[kk][position]*(t_node+t_dis);
...
//            } printf("\n");
            ...
            for(int j = 0; j < m; j ++) {
                if(!individual->mesh[machineMap[_m][j]]) {
                    for(int i = 0; i < individual->machine[_id1].size() / 2; i ++) {
                        individual->machine[machineMap[_m][j]].push_back(individual->machine[_id1][i]);
                    }
            ...
            for(int j = 0; j < m; j ++) {
                if(!individual->mesh[machineMap[_m][j]]) {
                    for(int i = individual->machine[_id1].size() / 2; i < individual->machine[_id1].size(); i ++) {
                        individual->machine[machineMap[_m][j]].push_back(individual->machine[_id1][i]);
                    }
                    individual->mesh[machineMap[_m][j]] = 1;
                    break;
...
//            while(1) {}
            ...
            for(int i = 0; i < m; i ++) {
                if(!individual->mesh[i]) {
                    ...
                    for(int j = 0; j < individual->machine[_id1].size(); j ++) {
                        individual->machine[i].push_back(individual->machine[_id1][j]);
                    }
                    for(int j = 0; j < individual->machine[_id2].size(); j ++) {
                        individual->machine[i].push_back(individual->machine[_id2][j]);
                    }
                    individual->mesh[i] = 1;
                    break;
                }
            ...
            for(int i = 0; i < m; i ++) {
                if(machineMap[_m][i] == _id1 || machineMap[_m][i] == _id2) {
                    ...
                    for(int j = 0; j < individual->machine[_id].size(); j ++) {
                        individual->machine[machineMap[_m][i]].push_back(individual->machine[_id][j]);
                    }
                    break;
                }
            }
            individual->machine[_id].clear();
            individual->mesh[_id] = 0;
            for(int i = 0; i < m; i ++) {
                printf("%d ", individual->mesh[i]);
...
{
    if(a->makespan == b->makespan && a->p_contention == b->p_contention && a->energy==b->energy) return 0;//a=b
...
void relaxElistCollection(Individual *i)
{
    ...
    {
        if(i->makespan == (*iter).makespan && (i->p_contention == (*iter).p_contention)&& (i->energy == (*iter).energy))
        {
            continue;
        }
        else
...
//        printf("evaluate begin ********************aaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n");
    ...
    for(int i=0; i<meshfactor*meshfactor; i++)
    {
        for(int j=0; j<meshfactor*meshfactor; j++)
        {
            for(int p=0; p<meshfactor*meshfactor; p++)
            {
                ...
                {
                    ox=(j/meshfactor-i/meshfactor)+(q/meshfactor-p/meshfactor)-((j/meshfactor)>(q/meshfactor)?(j/meshfactor):(q/meshfactor))+((i/meshfactor>p/meshfactor)?(p/meshfactor):(i/meshfactor));
                    oy=(j%meshfactor-i%meshfactor)+(q%meshfactor-p%meshfactor)-((j%meshfactor)>(q%meshfactor)?(j%meshfactor):(q%meshfactor))+((i%meshfactor>p%meshfactor)?(p%meshfactor):(i%meshfactor));
                    if(ox<0)
                    {
                        ox=0;
                    }
...
//读文件
    ...
    for(int j=1; j<meshfactor*meshfactor; j++)
    {
        ...
        {
            factor_construction=factor_construction/2.0;
            Rdk[j]=Rdk[j]*2.0;
        }
        for(int i=0; i<n; i++)
        {
